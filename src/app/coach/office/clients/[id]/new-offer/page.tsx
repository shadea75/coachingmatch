      ...
      try {
        ...
        if (isCoachaMiClient && coacheeId) {
          ...
          if (userDoc.exists()) {
            ...
            // Se email mancante, cerca nella collection leads
            if (!clientEmail) {
              try {
                const leadsSnap = await getDocs(query(collection(db, 'leads'), where('assignedCoachId', '==', user.id)))
                for (const ld of leadsSnap.docs) {
                  if (ld.data().name?.toLowerCase() === (data.name || '').toLowerCase()) {
                    clientEmail = ld.data().email
                    break
                  }
                }
              } catch (e) { console.error('Errore ricerca email lead:', e) }
            }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isCoachaMiClient && coacheeId) {
        ...
        console.log('Invio email offerta a:', client.email, client.name)
        if (sendEmail && client.email) {
          try {
            ...
            console.log('Risultato invio email offerta:', emailResult)
          } catch (emailErr) {
            console.error('Errore invio email:', emailErr)
          }
        } else if (sendEmail && !client.email) {
          console.warn('Email non inviata: indirizzo email mancante per il cliente')
        }
